...
//         // Just refresh the list instead of manually inserting
...
//         // Just refresh the list instead of manually updating
//         await _refreshDiaries();
//       }
//     } catch (e) {
...
class _DiaryPageState extends State<DiaryPage> {
  ...
  /// Refresh the diary list after a new entry or an update
  Future<void> _refreshDiaries() async {
    try {
      print('Refreshing diary list...');
      await _loadDiaries();
    } catch (e) {
      print('Error refreshing diaries: $e');
      if (mounted) {
  ...
  /// Open NewDiaryPage to create a new diary entry.
  Future<void> _navigateToNewDiary() async {
    try {
      final result = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => const NewDiaryPage(), // For new diary
        ),
      );
      if (result != null && mounted) {
        print('New diary created, refreshing list');
  ...
  /// Open an existing diary entry in NewDiaryPage for viewing or editing.
  Future<void> _openDiary(int index) async {
    try {
      print('Opening diary at index $index with data: ${diaries[index]}');
      ...
      Map<String, dynamic> serializedDiary = {
        'id': diaryData['id'],
        'title': diaryData['title']?.toString() ?? 'No Title',
        'content': diaryData['content']?.toString() ?? '',
        'date': diaryData['date'],
        'background_color': diaryData['background_color'],
        'text_color': diaryData['text_color'],
        'backgroundColor': diaryData['background_color'], // Backward compatibility
        'textColor': diaryData['text_color'],             // Backward compatibility
        'mood': diaryData['mood'] ?? '',
        'images': diaryData['images'] ?? [],
        'template': diaryData['template'] ?? 'Default',
        'isEditing': true, // Indicates this is an existing diary
      };
      ...
      final result = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => NewDiaryPage(initialData: serializedDiary),
        ),
      );
      if (result != null && mounted) {
        print('Diary updated, refreshing list');
  ...
  /// Build a card for recent diary entries.
  Widget _buildRecentDiaryCard(int index) {
    if (index >= diaries.length) {
      // "New Entry" card
      return Card(
  ...
  /// Build the list view for all diary entries.
  Widget _buildDiaryListItem(int index) {
    final diary = diaries[index];
    print('Building diary list item for index $index: $diary');
    final date = DateTime.parse(diary['date']);

    // Simple list item displaying the diary title and date.
    return ListTile(
      leading: Container(
        width: 40,
        height: 40,
        ...
        child: const Center(
          child: Icon(Icons.auto_stories, size: 24, color: Color(0xFF255DE1)),
        ),
      ),
      title: Text(
        diary['title']?.toString() ?? DateFormat('MMM d, yyyy').format(date),
