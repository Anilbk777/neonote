...
class _InvitationInboxPageState extends State<InvitationInboxPage> {
  ...
  Future<void> deleteInvitation(Map<String, dynamic> invitation) async {
    final token = await LocalStorage.getToken();
    if (token == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Not authenticated')),
    ...
    final response = await http.delete(
      Uri.parse('http://localhost:8000/api/work/invitations/${invitation['id']}/delete/'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json',
      },
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      IconButton(
                                        ...
                                        onPressed: () => deleteInvitation(invitation),
                                      ),
                                    ],
                                  ),
